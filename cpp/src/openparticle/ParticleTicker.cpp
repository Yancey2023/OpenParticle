...
namespace OpenParticle {
    ...
    multiply(const std::optional<Eigen::Matrix4f> &matrix1, const std::optional<Eigen::Matrix4f> &matrix2) {
        if (!matrix1.has_value()) [[likely]] {
            return matrix2;
        } else if (!matrix2.has_value()) [[likely]] {
            return matrix1;
        }
    ...
    void Node::buildCache(int32_t tick,
        ...
        ASSERT_NOTNULL(particle)
        if (tick < tickStart || tick >= tickEnd) [[likely]] {
            return;
        }
        if (isTransformNode) [[likely]] {
            ASSERT_NOTNULL(children)
            ...
            if (parentCache.cachePosition.has_value()) [[likely]] {
    ...
    ParticleTicker::ParticleTicker(const ParticleData *particleData) {
        ...
        for (int i = 0; i < size; ++i) {
            if (!nodeList[i].isTransformNode) [[unlikely]] {
